...
$ python fairseq_cli/hydra_train.py \
  ...
  task.data=/path/to/data task.label_dir=["/path/to/labels"] task.labels=["km"] model.label_rate=[100.0]
```
To train a base model with multiple labels (in different frame rate), run:
```sh
$ python fairseq_cli/hydra_train.py \
  --config-dir /path/to/fairseq-py/examples/hubert/config/pretrain \
  --config-name hubert_base_librispeech \
  task.data=/path/to/data task.labels=["km", "km"] \
  +model.label_dir=["/path/to/label1", "/path/to/label2"]  \
  +model.label_rate=["100.0", "50.0"]  \
  +model.use_multiple_frame_rate_label=true
```
...
```
### Fine-tune a HuBERT model with a CE loss, for frame-wise phoneme classification.
To fine-tune a pre-trained HuBERT model at `/path/to/checkpoint`, using the corresponding label_rate, run
```sh
$ python fairseq_cli/hydra_train.py \
  --config-dir /path/to/fairseq-py/examples/hubert/config/finetune \
