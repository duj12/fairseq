...
def thread_main(tsv_dir, split, ckpt_path, layer, nshard, rank, feat_dir, max_chunk):
    reader = HubertFeatureReader(ckpt_path, layer, max_chunk)
    import threading
    threads = []
    for rank in range(nshard):
        generator, num = get_path_iterator(f"{tsv_dir}/{split}.tsv", nshard, rank)
        threads.append(threading.Thread(target=dump_feature, args=(reader, generator, num, split, nshard, rank, feat_dir)))
    for rank in range(nshard):
        threads[rank].start()
