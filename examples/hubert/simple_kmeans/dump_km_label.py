...
def thread_dump_label(feat_dir, split, km_path, nshard, rank, lab_dir):
    import threading
    apply_kmeans = ApplyKmeans(km_path)

    def sing_job(feat_dir, split, nshard, rank, lab_dir):
        generator, num = get_feat_iterator(feat_dir, split, nshard, rank)
        iterator = generator()
    ...
    for rank in range(nshard):
        threads.append(threading.Thread(target=sing_job, args=(feat_dir, split, nshard, rank, lab_dir)))
