...
cat $tgt_dir/all.uid | awk -v wav_dir1=$wav_dir '{print $1" "wav_dir1"/"$1".wav"}' | sort > $tgt_dir/all_wav.scp
...
for s in $setups; do
  ...
  for x in $splits; do
    ...
    ln -sf $tgt_dir/$s/$x.phn $tgt_dir/$s/$x.wrd
  ...
  ln -sf $tgt_dir/$s/dict.phn.txt $tgt_dir/$s/dict.wrd.txt
done
