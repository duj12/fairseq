...
class HubertPretrainingConfig(FairseqDataclass):
    ...
    use_multiple_frame_rate_label: Optional[bool] = field(
        default=False, metadata={"help": "set to true if using multiple labels with different frame rate"}
    )
    # 将label_dir改成list，以支持存储在不同路径下的标签
    label_dir_list: List[str] = field(
        default_factory=lambda: [""],
        metadata={
            "help": "if set, looks for labels in this directory instead",
        },
    )
    # 将label_rate改成list格式，顺序匹配labels的顺序，可以支持不同帧率的标签
    label_rate_list: List[float] = field(
...
class HubertPretrainingTask(FairseqTask):
    ...
    def load_dictionaries(self):
        ...
        if self.use_multiple_frame_rate_label:
            dictionaries = [
                Dictionary.load(f"{label_dir[i]}/dict.{label}.txt")
    ...
    def get_label_dir(self) -> str:
        if self.use_multiple_frame_rate_label:
            return self.cfg.label_dir_list
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        if self.use_multiple_frame_rate_label:  # 默认不同帧率的标签是放置在不同的文件夹下面，生成数据时会更方便一些
            paths = [f"{self.get_label_dir()[i]}/{split}.{l}" for i, l in enumerate(self.cfg.labels)]
        else:
            paths = [f"{self.get_label_dir()}/{split}.{l}" for i,l in enumerate(self.cfg.labels)]
        ...
        self.datasets[split] = HubertDataset(
            ...
            label_paths=paths,
            label_rates=self.cfg.label_rate if not self.use_multiple_frame_rate_label else self.cfg.label_rate_list,
            pad_list=pad_list,
            eos_list=eos_list,
