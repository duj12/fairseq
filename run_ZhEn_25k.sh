...
ckpt_path=/data/megastore/Projects/DuJing/model/HuBERT/hubert_base_ls960.pt    #hubert_large_ll60k.pt
layer=9   #参考hubert论文
nshard=20
rank=20   #rank==nshard时表示多线程并行，rank<nshard表示只运行其中一个线程
feat_dir=$tsv_dir/hubert-ls-eng
if [ ! -d  $feat_dir ]; then 
	mkdir -p $feat_dir
fi
n_cluster=500  
...
CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7"  \
	python -u fairseq_cli/hydra_train.py \
		...
		hydra.run.dir=outputs/ZhEn_25k-ppg+hubert+ls_eng-base \
		task.data=$tsv_dir \
		task.labels=[km,km,km] \
		+task.label_dir_list=['/data/megastore/Projects/DuJing/data/ZhEn_25k/ppg','/data/megastore/Projects/DuJing/data/ZhEn_25k/hubert','/data/megastore/Projects/DuJing/data/ZhEn_25k/hubert-ls-eng/cluster_500'] \
